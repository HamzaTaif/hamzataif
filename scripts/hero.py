...
def generate_portrait_elements(input_path="hamza.png", is_dark=True):
    ...
    for gy in range(grid_h):
        ...
        for gx in range(grid_w):
            ...
            if is_dark:
                if lum > 0.12:
                    r_dot = round((0.45 + lum * 1.05) * fade, 1)
                    if lum > 0.65:
                        col = "#F5F4F1"
                        op = round(min(1.0, (0.5 + lum * 0.5) * fade), 1)
                    elif lum > 0.35:
                        col = "#8B6F47"
                ...
                inv_lum = 1.0 - lum
                if inv_lum > 0.12:
                    r_dot = round((0.45 + inv_lum * 1.05) * fade, 1)
                    if inv_lum > 0.58:
                        col = "#1A1917"
                        op = round(min(1.0, (0.5 + inv_lum * 0.5) * fade), 1)
                    elif inv_lum > 0.3:
                        col = "#8B6F47"
...
def create_hero_svg(is_dark=True):
    ...
    portrait_group = f'''<g transform="translate(610, 22)">
    <style>
      .hero-portrait {{
    ...
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 330" width="100%" height="100%">
  <title>Hamza Taif — Software Engineering, AI, Full-Stack App Development</title>

  <!-- Background -->
  <rect x="0" y="0" width="900" height="330" fill="{bg}" />

  <!-- Left Accent Line -->
  <rect x="0" y="0" width="3" height="330" fill="{accent}" />

  <!-- Top Structural Line -->
  <line x1="48" y1="42" x2="560" y2="42" stroke="{line_color}" stroke-width="1" />

  <!-- Stacked Name Typography -->
  <text x="44" y="142" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" font-size="86" font-weight="900" fill="{text_primary}" letter-spacing="-1">HAMZA</text>
  <text x="44" y="226" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" font-size="86" font-weight="900" fill="{line_color}" letter-spacing="-1">TAIF</text>

  <!-- Accent T Overlay -->
  <text x="44" y="226" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" font-size="86" font-weight="900" fill="{accent}" letter-spacing="-1">T</text>

  <!-- Discipline Line -->
  <text x="47" y="270" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" font-size="13" font-weight="600" fill="{text_muted}" letter-spacing="1.2">SOFTWARE ENGINEERING  ·  AI  ·  FULL-STACK APP DEVELOPMENT</text>

  <!-- Tagline -->
  <text x="47" y="296" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" font-size="13" font-weight="400" fill="{text_primary}" opacity="0.75" letter-spacing="0.3">Building useful software, one system at a time.</text>

  <!-- Right Side Stipple Portrait -->
  {portrait_group}

  <!-- Bottom Structural Line -->
  <line x1="48" y1="312" x2="560" y2="312" stroke="{line_color}" stroke-width="1" />
</svg>'''
    return svg
