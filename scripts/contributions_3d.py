...
def draw_3d_cube(x, y, dx, dy, height, color_top, color_left, color_right, is_dark=True):
    # Isometric projections
    # dx, dy = grid cell steps
    # Isometric transform:
    # ISO_X = x - y
    # ISO_Y = (x + y) / 2 - height
    ...
    if height == 0:
        return f'<polygon points="{top_poly}" fill="{color_top}" stroke="#2A2925" stroke-width="0.6" />\n'
    ...
    return f'''<polygon points="{left_poly}" fill="{color_left}" stroke="#1A1917" stroke-width="0.5" />
<polygon points="{right_poly}" fill="{color_right}" stroke="#1A1917" stroke-width="0.5" />
<polygon points="{top_poly}" fill="{color_top}" stroke="#1A1917" stroke-width="0.5" />\n'''

def create_3d_contributions_svg(is_dark=True):
    ...
    for week in range(40):
        for day in range(7):
            ...
            cubes_xml += draw_3d_cube(px, py, 8, 4, h, c_top, c_left, c_right, is_dark)
  ...
  <!-- Side Statistics Block -->
  {stats_block}

  <!-- 3D Isometric Columns Grid -->
  <g>
    {cubes_xml}
  </g>
</svg>'''
...
def main():
    ...
    with open("assets/contributions-3d-dark.svg", "w", encoding="utf-8") as f:
        f.write(create_3d_contributions_svg(is_dark=True))
    with open("assets/contributions-3d-light.svg", "w", encoding="utf-8") as f:
        f.write(create_3d_contributions_svg(is_dark=False))
    print("Generated assets/contributions-3d-dark.svg and assets/contributions-3d-light.svg (3D Isometric Calendar)")
